document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ------------------------------------------------
   * INFINITE (SEAMLESS) CAROUSELS — robust no-stuck
   * ------------------------------------------------ */
  document.querySelectorAll('.carousel-container').forEach((carousel) => {
    ...
    const measure = () => Math.round(viewport.getBoundingClientRect().width);
    ...
    const enableTransition  = () => { track.style.transition = 'transform 0.5s ease'; };
    ...
    const layout = () => {
      const w = measure();
      if (!w) { // viewport not laid out yet → retry soon
        clearTimeout(layout._retry);
        layout._retry = setTimeout(layout, 50);
        return;
      }
      width = w;
    ...
    const next = () => {
      ...
      isAnimating = true;
      enableTransition();
      // ensure transform actually changes on a new frame
      requestAnimationFrame(() => {
        index += 1;
        setTransform();
        // watchdog in case transitionend never fires
        clearTimeout(watchdog);
        watchdog = setTimeout(finishTransition, 1200);
      });
    };
    ...
    const goToReal = (realIdx) => {
      ...
      enableTransition();
      requestAnimationFrame(() => {
        index = realIdx + 1; // account for leading clone
        setTransform();
        clearTimeout(watchdog);
        watchdog = setTimeout(finishTransition, 1200);
      });
    ...
    // Only react to transform on the track
    track.addEventListener('transitionend', (e) => {
      if (e.target !== track || e.propertyName !== 'transform') return;
      finishTransition();
    });
    track.addEventListener('transitioncancel', finishTransition);
    ...
    // Wait for images (or proceed if none)
    ...
    // Keep layout correct on size changes
